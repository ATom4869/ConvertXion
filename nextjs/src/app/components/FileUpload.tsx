...
export default function FileUpload() {
    ...
    const [keepOriginalResolution, setKeepOriginalResolution] = useState<boolean>(true);
    ...
    const handleProcessSingleFile = async (file: File) => {
        ...
        try {
            ...
            if (currentFormat?.useQuality) formData.append("quality", quality.toString());
            if (currentFormat?.useCompression) formData.append("compression", compression.toString());
    ...
    const handleProcessMultipleFiles = async (files: File[]) => {
        ...
        const socket = new WebSocket(`ws://localhost:5100/api/ws?session_id=${sessionID}`);
        ...
        socket.onmessage = (event) => {
            try {
                ...
                if (currentFile && !currentFile.includes('started') && !currentFile.includes('zip')) {
                    ...
                    <div className="flex items-center justify-center gap-2 text-sm text-accent-600 mt-1">
                        {isZipping ? (
                            ...
                            <span>
                                ...
                                {progress >= 10 && progress < 90 && !isZipping && "Converting files..."}
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5100/api/convert?session_id=${sessionID}`, {
                method: "POST",
                body: formData,
                mode: "cors",
                headers: {
    ...
    const handleWidthChange = (value: number | string) => {
        ...
        if (keepAspectRatio && selectedAspectRatio !== null && typeof newWidth === 'number') {
    ...
    const handleHeightChange = (value: number | string) => {
        ...
        if (keepAspectRatio && selectedAspectRatio !== null && typeof newHeight === 'number') {
    ...
    const handleAvifCompressionChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Find the current format details
    const currentFormat = SUPPORTED_FORMATS.find(f => f.value === format) || SUPPORTED_FORMATS[0];
    ...
    return (
        <div className="flex flex-col w-full items-center min-h-screen bg-base-100">
            <main className="flex flex-col items-center flex-grow p-4">
                ...
                >
                    <input {...getInputProps()} type="file" ref={inputRef} style={{ display: "none" }} className="file-input file-input-accent" />
                    <br />
                    <p className="text-base-content">Drag & drop files here</p>
                    <p className="text-base-content/70 text-sm mt-1">Support for single or multiple files</p>
                    <button
                ...
                {files.length > 0 && (
                    <div className="mt-4 text-center w-full max-w-2xl">
                        ...
                        <div className="mt-2">
                            ...
                            <div className="mt-3 flex justify-between items-center">
                                <p className="text-base-content/90 font-medium">
                                    {files.length > 1 && "Multiple files will be converted and delivered as a zip file"}
                                </p>
                                <button
                        ...
                        <div className="mt-6 bg-base-200 p-4 rounded-lg border border-base-300">
                            <h3 className="text-lg font-medium text-base-content mb-3">Conversion Settings</h3>

                            <div className="mb-4">
                                <label className="block text-base-content font-medium mb-2">Output Format</label>
                                <select
                            ...
                            <div className="mb-4">
                                <label className="flex items-center cursor-pointer mb-2">
                                    ...
                                    />
                                    <span className="text-accent-700 font-medium">Keep Original Resolution</span>
                                </label>

                                {!keepOriginalResolution && (
                                    <div className="pl-6 pt-2">
                                        <div className="mb-3">
                                            <label className="block text-accent-700 mb-1">Resolution Presets</label>
                                            <select
                                        ...
                                        <div className="mb-3">
                                            <label className="flex items-center cursor-pointer mb-2">
                                                <input
                                                    ...
                                                    className="toggle toggle-accent mr-2"
                                                />
                                                <span className="text-accent-700">Maintain Aspect Ratio</span>
                                            </label>

                                            {keepAspectRatio && (
                                                <div className="mb-3">
                                                    <label className="block text-neutral mb-1">Aspect Ratio</label>
                                                    <div className="flex gap-2">
                                                        {ASPECT_RATIOS.map((ratio) => (
                                                            <button
                                                                key={ratio.label}
                                                                type="button"
                                                                className={`btn px-3 py-1 border hover:btn-accent-focus rounded ${selectedAspectRatio === ratio.value
                                            ...
                                            <div className="grid grid-cols-2 gap-2">
                                                <div>
                                                    <label className={`block text-sm ${theme === 'cupcake' ? 'text-neutral' : 'text-primary'} mb-1`}>
                                                        Width (px)
                                                    </label>
                                                    <input
                                                        type="number"
                        ...
                        <div className="mt-6">
                            ...
                            {loading && (
                                <div className="w-full mt-4">
                                    ...
                                    <p className="text-sm text-accent-600 mt-1">Processing... {progress}%</p>
                                </div>
                            )}
                            ...
                            {progress >= 100 && convertedFile && (
                                <ProcessBtn
                                    text={files.length === 1 ? "Download Converted File" : "Download Converted Files (ZIP)"}
